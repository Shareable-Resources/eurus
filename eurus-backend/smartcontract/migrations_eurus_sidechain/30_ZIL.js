...
module.exports = async function (deployer) {
    ...
    await deployHelper.deployWithProxy(deployer, bnb, 'ZIL', ownedProxy)
    ...
    // let err= await bnbobj.methods.init(internalSCConfig.address, "ZIL", "ZIL", 114690000000000000000000000n, 12).send(await deployHelper.callParams(accounts));
    // console.log("init function tx receipt: " + JSON.stringify(err));

    // console.log("Register ZIL to ExternalSmartContractConfig" );
    ...
    // await externalScObj.methods.removeCurrencyInfo("ZIL").send(await deployHelper.callParams(accounts));
    // await externalScObj.methods.addCurrencyInfo(ownedProxyInstance.address, "ZIL").send(await deployHelper.callParams(accounts));
