...
module.exports = async function (deployer) {
    ...
    await deployHelper.deployWithProxy(deployer, bnb, 'cDAI', ownedProxy)
    ...
    // let err= await bnbobj.methods.init(internalSCConfig.address, "cDAI", "cDAI", 114690000000000000000000000n, 8).send(await deployHelper.callParams(accounts));
    // console.log("init function tx receipt: " + JSON.stringify(err));

    // console.log("Register cDAI to ExternalSmartContractConfig" );
    ...
    // await externalScObj.methods.removeCurrencyInfo("cDAI").send(await deployHelper.callParams(accounts));
    // await externalScObj.methods.addCurrencyInfo(ownedProxyInstance.address, "cDAI").send(await deployHelper.callParams(accounts));
