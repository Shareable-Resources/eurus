...
module.exports = async function (deployer) {
    ...
    await deployHelper.deployWithProxy(deployer, bnb, 'OMG', ownedProxy)
    ...
    // let err= await bnbobj.methods.init(internalSCConfig.address, "OMG", "OMG", 114690000000000000000000000n, 18).send(await deployHelper.callParams(accounts));
    // console.log("init function tx receipt: " + JSON.stringify(err));

    // console.log("Register OMG to ExternalSmartContractConfig" );
    ...
    // await externalScObj.methods.removeCurrencyInfo("OMG").send(await deployHelper.callParams(accounts));
    // await externalScObj.methods.addCurrencyInfo(ownedProxyInstance.address, "OMG").send(await deployHelper.callParams(accounts));
