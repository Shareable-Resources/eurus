...
module.exports = async function (deployer) {
    ...
    await deployHelper.deployWithProxy(deployer, bnb, 'HT', ownedProxy)
    ...
    // let err= await bnbobj.methods.init(internalSCConfig.address, "HT", "HT", 114690000000000000000000000n, 18).send(await deployHelper.callParams(accounts));
    // console.log("init function tx receipt: " + JSON.stringify(err));

    // console.log("Register HT to ExternalSmartContractConfig" );
    ...
    // await externalScObj.methods.removeCurrencyInfo("HT").send(await deployHelper.callParams(accounts));
    // await externalScObj.methods.addCurrencyInfo(ownedProxyInstance.address, "HT").send(await deployHelper.callParams(accounts));
