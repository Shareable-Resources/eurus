...
module.exports = async function (deployer) {
    ...
    await deployHelper.deployWithProxy(deployer, bnb, 'DAI', ownedProxy)
    ...
    // let err= await bnbobj.methods.init(internalSCConfig.address, "DAI", "DAI", 114690000000000000000000000n, 18).send(await deployHelper.callParams(accounts, deployer));
    // console.log("init function tx receipt: " + JSON.stringify(err));

    // console.log("Register DAI to ExternalSmartContractConfig" );
    ...
    // await externalScObj.methods.removeCurrencyInfo("DAI").send(await deployHelper.callParams(accounts, deployer));
    // await externalScObj.methods.addCurrencyInfo(ownedProxyInstance.address, "DAI").send(await deployHelper.callParams(accounts, deployer));
