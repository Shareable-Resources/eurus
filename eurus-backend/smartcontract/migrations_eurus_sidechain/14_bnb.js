...
module.exports = async function (deployer) {
    ...
    await deployHelper.deployWithProxy(deployer, bnb, 'BNB', ownedProxy)
    ...
    // let err= await bnbobj.methods.init(internalSCConfig.address, "BNB", "BNB", 13242013521397079n, 18).send(await deployHelper.callParams(accounts, deployer));
    ...
    // await externalScObj.methods.removeCurrencyInfo("BNB").send(await deployHelper.callParams(accounts, deployer));
    // await externalScObj.methods.addCurrencyInfo(ownedProxyInstance.address, "BNB").send(await deployHelper.callParams(accounts, deployer));
