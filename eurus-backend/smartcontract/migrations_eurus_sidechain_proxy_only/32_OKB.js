...
module.exports = async function (deployer) {
    ...
    await deployHelper.deployWithProxy(deployer, bnb, 'OKB', ownedProxy)
    ...
    // let err= await bnbobj.methods.init(internalSCConfig.address, "OKB", "OKB", 114690000000000000000000000n, 18).send(await deployHelper.callParams(accounts));
    // console.log("init function tx receipt: " + JSON.stringify(err));

    // console.log("Register OKB to ExternalSmartContractConfig" );
    ...
    // await externalScObj.methods.removeCurrencyInfo("OKB").send(await deployHelper.callParams(accounts));
    // await externalScObj.methods.addCurrencyInfo(ownedProxyInstance.address, "OKB").send(await deployHelper.callParams(accounts));
