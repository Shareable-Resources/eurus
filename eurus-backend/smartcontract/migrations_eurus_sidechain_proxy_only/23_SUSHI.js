...
module.exports = async function (deployer) {
    ...
    await deployHelper.deployWithProxy(deployer, bnb, 'SUSHI', ownedProxy)
    ...
    // let err= await bnbobj.methods.init(internalSCConfig.address, "SUSHI", "SUSHI", 114690000000000000000000000n, 18).send(await deployHelper.callParams(accounts));
    // console.log("init function tx receipt: " + JSON.stringify(err));

    // console.log("Register SUSHI to ExternalSmartContractConfig" );
    ...
    // await externalScObj.methods.removeCurrencyInfo("SUSHI").send(await deployHelper.callParams(accounts));
    // await externalScObj.methods.addCurrencyInfo(ownedProxyInstance.address, "SUSHI").send(await deployHelper.callParams(accounts));
