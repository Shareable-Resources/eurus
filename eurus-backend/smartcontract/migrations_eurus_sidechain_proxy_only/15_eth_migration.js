...
var etherc20 = artifacts.require("EurusERC20");
...
module.exports = async function (deployer) {
    ...
    await deployHelper.deployWithProxy(deployer, etherc20, 'ETH', ownedProxy)

    await etherc20.deployed();
    ...
    // let err= await bnbobj.methods.init(internalSCConfig.address, "ETH", "ETH", 114690000000000000000000000n, 18).send(await deployHelper.callParams(accounts, deployer));
    ...
    // await externalScObj.methods.removeCurrencyInfo("ETH").send(await deployHelper.callParams(accounts, deployer));
    // await externalScObj.methods.addCurrencyInfo(ownedProxyInstance.address, "ETH").send(await deployHelper.callParams(accounts, deployer));
