...
module.exports = async function (deployer) {
    ...
    await deployHelper.deployWithProxy(deployer, bnb, 'USDK', ownedProxy)
    ...
    // let err= await bnbobj.methods.init(internalSCConfig.address, "USDK", "USDK", 114690000000000000000000000n, 18).send(await deployHelper.callParams(accounts));
    ...
    // await externalScObj.methods.removeCurrencyInfo("USDK").send(await deployHelper.callParams(accounts));
    // await externalScObj.methods.addCurrencyInfo(ownedProxyInstance.address, "USDK").send(await deployHelper.callParams(accounts));
