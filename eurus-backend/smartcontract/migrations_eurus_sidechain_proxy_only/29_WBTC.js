...
module.exports = async function (deployer) {
    ...
    await deployHelper.deployWithProxy(deployer, bnb, 'WBTC', ownedProxy)
    ...
    // let err= await bnbobj.methods.init(internalSCConfig.address, "WBTC", "WBTC", 114690000000000000000000000n, 8).send(await deployHelper.callParams(accounts));
    // console.log("init function tx receipt: " + JSON.stringify(err));

    // console.log("Register WBTC to ExternalSmartContractConfig" );
    ...
    // await externalScObj.methods.removeCurrencyInfo("WBTC").send(await deployHelper.callParams(accounts));
    // await externalScObj.methods.addCurrencyInfo(ownedProxyInstance.address, "WBTC").send(await deployHelper.callParams(accounts));
