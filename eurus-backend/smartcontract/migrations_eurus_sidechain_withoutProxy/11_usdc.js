...
module.exports = async function (deployer) {
    ...
    await deployHelper.deploy(deployer,usdc,"USDC")
    let usdcInstance = await usdc.deployed();
    let accounts = await web3.eth.getAccounts();


    let ownedProxyObj = deployHelper.getSmartContractInfoByName(deployer, "OwnedUpgradeabilityProxy<USDC>");
    let ownedProxyInstance = new web3.eth.Contract(ownedProxyJson.abi,ownedProxyObj.address);
    await ownedProxyInstance.methods.upgradeTo(usdcInstance.address).send(await deployHelper.callParams(accounts, deployer));
    //
    //
    //
    // let blacklistSC = deployHelper.getSmartContractInfoByName(deployer, "OwnedUpgradeabilityProxy<USDC>");
