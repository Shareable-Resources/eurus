...
var wbtc = artifacts.require("EurusERC20");
...
module.exports = async function (deployer) {
    ...
    await deployHelper.deploy(deployer,wbtc,"WBTC")
    let wbtcInstance = await wbtc.deployed();
    let accounts = await web3.eth.getAccounts();


    let ownedProxyObj = deployHelper.getSmartContractInfoByName(deployer, "OwnedUpgradeabilityProxy<WBTC>");
    let ownedProxyInstance = new web3.eth.Contract(ownedProxyJson.abi,ownedProxyObj.address);
    await ownedProxyInstance.methods.upgradeTo(wbtcInstance.address).send(await deployHelper.callParams(accounts, deployer));



    // let blacklistSC = deployHelper.getSmartContractInfoByName(deployer, "OwnedUpgradeabilityProxy<WBTC>");
