...
var omg = artifacts.require("EurusERC20");
...
module.exports = async function (deployer) {
    ...
    await deployHelper.deploy(deployer,omg,"OMG")
    let omgInstance = await omg.deployed();
    let accounts = await web3.eth.getAccounts();


    let ownedProxyObj = deployHelper.getSmartContractInfoByName(deployer, "OwnedUpgradeabilityProxy<OMG>");
    let ownedProxyInstance = new web3.eth.Contract(ownedProxyJson.abi,ownedProxyObj.address);
    await ownedProxyInstance.methods.upgradeTo(omgInstance.address).send(await deployHelper.callParams(accounts, deployer));


    //
    // let blacklistSC = deployHelper.getSmartContractInfoByName(deployer, "OwnedUpgradeabilityProxy<OMG>");
