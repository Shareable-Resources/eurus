...
async function getGlobalData(ethPrice, oldEthPrice) {
  ...
  try {
    ...
    const utcOneDayBack = 1639128060
    const utcTwoDaysBack = 1639128060
    const utcOneWeekBack = 1639128060
    const utcTwoWeeksBack = 1639128060
    // const utcOneDayBack = utcCurrentTime.subtract(1, 'day').unix()
    // const utcTwoDaysBack = utcCurrentTime.subtract(2, 'day').unix()
    // const utcOneWeekBack = utcCurrentTime.subtract(1, 'week').unix()
    // const utcTwoWeeksBack = utcCurrentTime.subtract(2, 'week').unix()
    // use 1638958680 for the test of GET NEAREST BLOCKS FOR NOW
    let nearestOneDayBlock = await client.query({
      query: GET_NEAREST_BLOCKS,
      variables: {
        timestampFrom: utcOneDayBack,
        timestampUntil: utcOneDayBack + 600,
      },
      fetchPolicy: 'cache-first',
    })
    let oneDayBlock = nearestOneDayBlock.data.transactions[0].blockNumber

    let nearestTwoDayBlock = await client.query({
      query: GET_NEAREST_BLOCKS,
      variables: {
        timestampFrom: utcTwoDaysBack,
        timestampUntil: utcTwoDaysBack + 600,
      },
      fetchPolicy: 'cache-first',
    })
    let twoDayBlock = nearestTwoDayBlock.data.transactions[0].blockNumber

    let nearestOneWeekBack = await client.query({
      query: GET_NEAREST_BLOCKS,
      variables: {
        timestampFrom: utcOneWeekBack,
        timestampUntil: utcOneWeekBack + 600,
      },
      fetchPolicy: 'cache-first',
    })
    let oneWeekBlock = nearestOneWeekBack.data.transactions[0].blockNumber

    let nearestTwoWeeksBack = await client.query({
      query: GET_NEAREST_BLOCKS,
      variables: {
        timestampFrom: utcTwoWeeksBack,
        timestampUntil: utcTwoWeeksBack + 600,
      },
      fetchPolicy: 'cache-first',
    })
    let twoWeekBlock = nearestTwoWeeksBack.data.transactions[0].blockNumber
