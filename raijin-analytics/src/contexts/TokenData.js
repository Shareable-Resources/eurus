...
const getTopTokens = async (ethPrice, ethPriceOld) => {
  // const currentDate = parseInt(Date.now() / 86400 / 1000) * 86400 - 86400 //real current date but currently not returning any new data
  const currentDate = parseInt((Date.now() / 86400 / 1000) * 86400 - 86400) - 26588800 // date six months ago
  ...
  // Current date using six months ago as it is not updated, rendering correctly
  try {
    ...
    let bulkResults = await Promise.all(
      current &&
        ...
        current?.data?.tokens.map(async (token) => {
          ...
          // console.log("getTopTokens data", data)
          return data
        })
    )
