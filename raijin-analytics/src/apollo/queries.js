...
export const GET_NEAREST_BLOCKS = gql`
  query transactions($timestampFrom: Int!, $timestampUntil: Int!) {
    transactions(
      first: 1
      orderBy: timestamp
      orderDirection: asc
      where: { timestamp_gt: $timestampFrom, timestamp_lt: $timestampUntil }
    ) {
      blockNumber
      timestamp
    }
