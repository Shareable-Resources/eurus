  ...
  <div class="login-container">
    ...
    >
      ...
      <el-button
        ...
        @click.native.prevent="loginWithMetamask"
      >
        Login with Metamask
      </el-button>
    </el-form>
...
import { setAddress, setToken } from '@/utils/auth'
  ...
  methods: {
    ...
    loginWithMetamask: async function() {
      ...
      try {
        ...
        if (isDappbrowser) {
          const getServerConfigResult = await getServerConfig()
          if (getServerConfigResult) {
            const switchResult = await switchEthereumChain()
            if (switchResult) {
              const accounts = await getAccount()
              const accountAddress = accounts[0]
              setAddress(accountAddress)
              const importWalletResponse = await importWallet(accountAddress)
              if (
