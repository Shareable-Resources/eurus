-- Deploy eurus-backend-db-report:20211101_report_audit_daily_server_usage_change_decimal to pg

BEGIN;

CREATE OR REPLACE FUNCTION public.report_audit_daily_server_usage(var_selected_date date)
 RETURNS TABLE(p_wallet_type character varying, p_used numeric(78,18), p_asset_name character varying, p_created_date date)
 LANGUAGE plpgsql
AS $function$
begin
return query	 
