...
CREATE OR REPLACE FUNCTION public.report_audit_cal_bal_change_select(var_selected_date date, var_wallet_type integer)
 RETURNS TABLE(p_asset_name character varying, p_selected_date date, 
 	p_created_date timestamp with time zone, p_last_modified_date timestamp with time zone, 
 	p_yesterday_balance numeric, p_selected_date_balance numeric, 
 	p_balance_change numeric, p_wallet_type integer)
 LANGUAGE plpgsql
AS $function$
begin
return query	 
